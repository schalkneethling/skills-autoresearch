...
1. Read the eval case prompt, `key_expectations`, and `scoring_dimensions`.
2. Read the producer's HTML output.
3. Score every dimension listed in the eval case's `scoring_dimensions` on the 0–3 scale defined above. Do not invent dimensions that are not listed for the case.
4. For each dimension, write a short rationale (1–2 sentences) that cites concrete details from the producer output.
5. Note which `key_expectations` were met or missed in the per-dimension rationales or the overall `summary`.
6. Sum the dimension scores into `total_score`; `max_score` is the sum of each dimension's `max_score` (3 per dimension on this rubric).

## Output Format

Return a single JSON object matching the harness `EvalScore` schema:

```json
{
  "eval_id": "product-listing-page",
  "eval_type": "generate-semantic-html",
  "track_id": "html",
  "total_score": 13,
  "max_score": 15,
  "dimensions": [
    {
      ...
      "score": 3,
      "max_score": 3,
      "rationale": "Every interactive control is a native element; <article> is used for product cards."
    },
    {
      "id": "aria_discipline",
      "score": 2,
      "max_score": 3,
      "rationale": "Nav labels are correct, but 'Add to cart' buttons share an identical accessible name."
    }
  ],
  "summary": "Strong landmarks and lists; ARIA discipline weakened by non-distinct button names."
}
```

Field rules:

- `eval_id`, `eval_type`, and `track_id` must match the eval case being scored.
- `dimensions` includes exactly the `scoring_dimensions` from the eval case — same `id`, same `max_score`.
- `total_score` equals the sum of dimension `score` values; the harness normalizes `total_score / max_score`.
- Do not emit a `null` score, a separate `composite_score`, `expectations_met/missed`, or any field not in this schema.
