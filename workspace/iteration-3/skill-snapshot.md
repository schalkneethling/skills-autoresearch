...
**Multi-step forms:** If you add a heading inside one `<fieldset>` step, add one inside all of them. Inconsistent structure (step 3 has an `<h2>`, steps 1–2 don't) creates an uneven document outline. Either use headings in every step or rely on the `<legend>` alone for all steps—don't mix approaches.
